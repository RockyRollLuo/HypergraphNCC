...
public class DecompositionDegree {
    private static final Logger LOGGER = Logger.getLogger(DecompositionDegree.class);

    private final Hypergraph hypergraph;
    private HashMap<Integer, Integer> coreVMap;
    private HashMap<Integer, Integer> coreEMap;

    public DecompositionDegree(Hypergraph hypergraph) {
    ...
    public Result run() {
            ...
            while (!deleteNodes.isEmpty()) {
                ...
                for (Integer v : deleteNodes) {
                    ...
                    for (Integer eId : tempNodeToEdgesMap.get(v)) {
                        ...
                        tempEdgeMap.remove(eId); //！！this edge has been deleted
                    }
        ...
        this.coreEMap = ComputeUtils.computeCoreEMapByCoreVMap(tempEdgeMap, tempCoreVMap);
        long endTime = System.nanoTime();
        double takenTime = (endTime - startTime) / 1.0E9D;
        LOGGER.info(takenTime);

        return new Result(coreVMap, coreEMap, takenTime, "DecompositionDegree");
    }
